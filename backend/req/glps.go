...
func sortBySTEM(plans []*types.GamifiedLessonPlan, order SortingOrder) ([]*types.GamifiedLessonPlan, error) {
	...
	for _, plan := range plans {
		...
		// todo do some crazy parse to check
		// what the domain is without parsing
		// the entire Contents value into a json object
...
func GetGLPSRequest() gin.HandlerFunc {
	return func(s *gin.Context) {
		...
		if indexQuery != "" {
			...
			result := api.GetGamifiedLessonPlans(s)
			...
			err := jsoniter.Unmarshal([]byte(result), &plans)
			if err != nil {
